# 행복도서관 프로젝트의 주요 기능에 관한 문서
 > 최초 작성 일자: 2024년 01월 15일 월요일 오후 02시 32분  
 > 최근 수정 일자: 2024년 02월 23일 금요일 오후 08시 31분
</br>
...
- **사용된 기술**
  ...
  - 휴대폰 인증 API를 활용한 본인인증 구현
  - 주소 API(react-daum-postcode)를 통해 주소 정보 입력
  - Redux을 사용하여 로그인 상태 관리
- **세부 내용**
  - 홈페이지 우측 상단의 로그인과 회원가입 링크를 클릭하여 각 입력폼으로 접근 가능
  - 회원가입 시 주소 API를 통해 주소 검색 후 자동 입력 
  - 회원 정보를 기입하고 휴대폰 인증 후 회원가입 가능
  - 휴대폰 인증 API를 활용하여 보안코드를 문자로 전송하고 DB에 저장한 후 사용자가 입력한 보안코드가 이에 일치하면 본인인증 완료
  ...
  - 아이디 및 비밀번호 찾기는 휴대폰 인증을 통해 본인확인 절차를 거친 후 진행 가능하도록 구현
...
### 6. 희망도서 신청
- **기능 요약**
  - 사용자가 추가되길 원하는 도서를 도서관측에 요청하는 기능
  - 사용자가 신청 양식에 희망도서 신청 내용을 작성하여 제출하면 관리자에게 그 내용이 전달되어 검토 진행
  - 관리자는 신청 내용을 검토하고 진행현황을 관리함
- **사용된 기술**
  - React 컴포넌트로 신청 양식 구현
  - axios를 통해 서버와 비동기 통신
- **세부 내용**
  - 사용자는 희망도서 신청 화면의 '신청하기' 버튼을 클릭하여 입력 폼으로 이동한 후 신청글 작성하여 제출
  - 사용자는 신청 입력 폼에 도서명, 저자 등 도서를 특정할 수 있는 내용을 입력하여 작성
  - 관리자는 신청 내용을 검토한 후 승인 시 '주문 -> 납품 -> 등록 -> 완료' 단계로 진행현황 관리
  - 관리자는 신청 내용을 검토한 후 취소한 경우 그 사유를 사용자에게 전달
  - 사용자는 신청 내용과 진행 현황을 '내 서재'에서 확인 가능
...
- **세부 내용**
  ...
  - 홈 화면에 최신 공지글 일부 표시
  - 공지사항 페이지에서 게시글 제목, 게시글 내용 등을 검색 조건으로 하여 게시글 검색 및 조회 가능하도록 기능 구현
